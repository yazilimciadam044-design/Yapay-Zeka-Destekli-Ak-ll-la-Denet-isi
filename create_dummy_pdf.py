...
"""
        ...
        {
            "filename": "matofin_prospektus.pdf",
            "content": """TIBBİ ÜRÜNÜN ADI: Matofin XR 500 mg Uzatılmış Salımlı Tablet (Metformin Hidroklorür)
1. KLİNİK ÖZELLİKLER
1.1 Terapötik Endikasyonlar:
Tip 2 Diabetes Mellitus tedavisinde, özellikle diyet ve egzersizin kan şekerini kontrol etmekte yetersiz kaldığı aşırı kilolu hastalarda ilk seçenek (first-line) antikosik ilaç olarak kullanılır. Polikistik Over Sendromu (PKOS) tedavisinde insülin direncini kırmak için off-label kullanılabilir.

1.2 Kontrendikasyonlar:
Metformine karşı aşırı duyarlılığı olanlar, diyabetik ketoasidoz ve diyabetik pre-koma durumları, şiddetli böbrek yetmezliği (GFR < 30 mL/dak), karaciğer yetmezliği, akut alkol zehirlenmesi ve dehidrasyon (şiddetli ishal, kusma) durumlarında kontrendikedir.

1.3 Özel Uyarılar:
Nadir ancak ölüm oranı yüksek (%50) Laktik Asidoz riski taşır (KIRMIZI ALARM). İyotlu kontrast maddelerle radyolojik inceleme yapılacaksa veya cerrahi müdahale öncesinde ilaç kesilmelidir. Uzun süreli kullanımda Vitamin B12 emilimini bozarak anemiye yol açabilir.

1.4 İstenmeyen Etkiler:
Çok Yaygın: Mide bulantısı, kusma, ishal, karın ağrısı ve iştah kaybı. Bu etkiler genellikle tedavinin başında görülür ve kendiliğinden geçer.
Seyrek: Ağızda metalik tat, eritem (cilt kızarıklığı), laktik asidoz.
Üretici Firma: Sanovel İlaç San. ve Tic. A.Ş. Menşei: Türkiye.
"""
        }
    ]
